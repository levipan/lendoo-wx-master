...
Page({
		...
		order.save().then(function (saveResult) {
			if (saveResult) {
				...
				for (var i = 0; i < carts.length; i++) {
					...
					orderGoodsMap.set('goods', AV.Object.createWithoutData('Goods', cart.goods.objectId));
					orderGoodsMap.set('quantity', cart.quantity);
					orderGoodsMap.set('user', cart.user);
					AV.Object.createWithoutData('Cart', cart.objectId).destroy();
		...
		query.find().then(function (address) {
			...
			for (var i = 0; i < address.length; i++) {
				...
				if (address[i].get('isDefault') == true) {
					that.setData({
						address: jsonify(address[i])
					})
				}
				addressList.push(address[i].detail);
			}
			that.setData({
	...
	pay: function ({orderId, totalFee}) {
		...
		AV.Cloud.run('pay', paramsJson).then(function(response) {
			...
			// 处理调用失败
			// 			code: 1
			// message: "商户涉嫌违规，已限制支付权限，请登录商户平台查看原因"
			// stack: "Error: 商户涉嫌违规，
			// 就是300块认证费没交，没续费而已
			console.log(err);
			if (err.code === 1) {
